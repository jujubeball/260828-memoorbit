...
- **작업 완료 후 사용자 배포 승인 요청**:
  - 코드 작성 및 검사가 완료되면 바로 Git 작업이나 배포를 수행하지 않고 사용자에게 확인을 구합니다.
  - 구현 및 검사 완료 후에는 먼저 7번의 **3단계 최종 보고 양식 전체**를 사용자에게 제공합니다.
  - 3단계 보고를 모두 제공한 다음 대화형 선택 버튼 UI인 `Ask for approval`을 실행합니다.
  - 사용자가 승인 버튼을 누르기 전에는 `git add`, `git commit`, `git push`, Vercel 배포 명령을 절대 실행하지 않습니다.
  - 사용자가 보류하거나 거절하면 작업 파일을 현재 상태로 보존하고, 커밋과 푸시가 대기 중임을 명확히 안내합니다.
- **대화형 선택 버튼 UI (`Ask for approval`) 필수 실행**:
  ...
  - 승인 요청에는 수정 파일, 검사 결과, 실행 예정인 한글 커밋 메시지와 푸시 대상 브랜치를 함께 명시합니다.
- **한글 Git 커밋 메시지 필수 규칙**:
...
## 7. 작업 완료 후 최종 보고 양식
작업 완료 시 반드시 아래 제목과 순서를 그대로 사용하여 답변합니다. 짧은 작업 요약이나 검사 결과만 남기고 설명을 끝내는 행위를 금지합니다.

1. **`# 1. 작업 요약 및 배포 확인 질문` (`Ask for approval` 버튼 UI 필수 실행)**
   - 수정된 파일 목록, 파일별 핵심 변경 내용, 타입 검사 및 린트 결과를 작성합니다.
   - 아직 커밋하거나 푸시하지 않았음을 명시하고, 실행 예정인 100% 한글 커밋 메시지를 함께 제시합니다.
   - 아래 2번과 3번 설명까지 모두 작성한 뒤 승인 버튼 팝업을 실행합니다.
...
3. **`# 3. [깊이 있는 프론트엔드 지식] 실무 핵심 개념 및 원리 돋보기`**
   ...
   - 승인 후 커밋과 푸시까지 완료했다면 같은 3단계 구조를 유지하면서 커밋 식별자와 푸시 결과를 1번 항목에 추가합니다.
