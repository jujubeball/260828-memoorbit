...
## 6. Git Push 승인 및 최종 보고 워크플로우 (Confirmation Workflow)

1. **로컬 작업 완료 시 (1차 승인 요청)**:
   - 긴 코드 설명이나 보고서 출력 없이, 수정된 파일 목록과 함께 `Ask for approval` 대화형 승인 버튼 UI로 **Git Push 승인**을 요청하십시오.

2. **사용자 승인 후 Git Push 완료 시 (2차 최종 보고서 출력)**:
   - 사용자가 승인하면 `git push origin main`을 즉시 실행하십시오.
   - Push가 정상 완료된 직후 아래 **3단계 최종 보고 양식**을 빠짐없이 작성하여 출력하십시오.

## 7. 작업 완료 후 최종 보고 양식 (Push 완료 후 출력)

1. **Git Push 완료 안내**:
   - `git push origin main` 완료 상태 및 커밋 메시지 안내.

2. **[상세 설명] 초등학생도 이해하는 꼼꼼한 코드 & 데이터 흐름 추적**:
   - 수정된 파일명, 컴포넌트명, 함수/State명을 명시하고 데이터가 이동하는 경로를 1단계, 2단계 순서로 쉬운 비유와 함께 설명.

3. **[깊이 있는 프론트엔드 지식] 실무 핵심 개념 및 원리 돋보기**:
   - 적용된 개념의 필요성, 안 쓸 때의 문제점, 실무 적용 사례를 풍부하게 설명.
