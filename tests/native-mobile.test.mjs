...
test("텍스트 목록은 썸네일 없이 제목·두 줄 요약·작성일·태그를 표시한다", async () => {
  const dom = setup();
  const { createRoot } = await import("react-dom/client");
  ...
  try {
    ...
    assert(article.querySelector("p").classList.contains("line-clamp-2"));
    assert.equal(article.querySelector("time").dateTime, memo.createdAt);
    assert(article.textContent.includes("#기록"));
    await act(async () => root.render(React.createElement(MemoCard, { ...props, memo, viewMode: "gallery" })));
    assert.equal(document.querySelector("img, svg"), null);
  } finally {
    await act(async () => root.unmount());
    dom.window.close();
  }
