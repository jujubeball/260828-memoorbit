...
export default function Home(): React.JSX.Element {
  ...
  // 연속 새 메모는 키를 바꿔 이전 편집기의 본문·선택·태그 상태를 함께 초기화합니다.
  ...
  // 저장 성공 또는 빈 메모에서만 호출하여 작성 중인 내용을 잃지 않고 새 캔버스를 엽니다.
    ...
    >
        ...
        {activeSection === "memos" && (
          <>
            ...
            {filteredMemos.length === 0 && (
              <p className="px-4 py-16 text-center text-sm text-slate-400" role="status">
                {memos.length === 0 ? "작성된 메모가 없습니다" : "조건에 맞는 메모가 없습니다"}
              </p>
            )}
            </div>
          </>
      ...
      {isEditorOpen && (
        <MemoModal
          ...
          key={`${editingMemo?.id ?? "new"}-${editorSession}`}
          isOpen
