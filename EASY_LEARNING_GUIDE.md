...
현재 설명 기준은 2026-09-11의 `21d8eda` 배포와 그 뒤의 Tag Orbit LOD 탐색 개편까지입니다. AI 연결이 없어도 태그와 작성일로 별자리를 만들고, 검색·태그 칩·성운 레이블로 메모 묶음을 탐색할 수 있습니다.
...
3. 궤도 화면은 그 목록을 별로 만들고, AI 관계가 없으면 태그와 작성일을 보고 로컬 별자리를 만듭니다.
4. 도화지에 크기와 밝기가 다른 별과 Glow 연결선을 그립니다. 별을 누르면 사진·날짜·태그가 있는 미리보기를 볼 수 있습니다.
...
**4단계 — AI가 없어도 로컬 별자리 만들기.** `OrbitGraphView`는 [orbitClustering.ts](src/lib/orbitClustering.ts)의 `createOrbitLayout`으로 시작 위치를 만듭니다. 먼저 `Memo.links`에 저장된 AI 관계를 확인합니다. 관계가 하나도 없다면 도서관 사서가 같은 주제 스티커가 붙은 책을 가까이 꽂듯이, 같은 태그가 있는 메모끼리 연결합니다. 공통 태그가 두세 개면 한 개일 때보다 더 튼튼한 고무줄로 묶습니다.

태그가 하나도 없는 메모는 버려두지 않습니다. 작성 날짜가 가장 가까운 메모 하나를 찾아 아주 가는 실로 연결합니다. 이 관계는 화면에서 길을 찾기 위한 임시 지도일 뿐이라 `Memo.links`나 브라우저 금고에 저장하지 않습니다. AI가 분석한 의미 관계와 로컬 Fallback 관계를 구분해야 하는 이유입니다.

그다음 `stepOrbitLayout`이 별을 조금씩 움직입니다. 연결된 별은 고무줄처럼 가까워지고, 겹치는 별은 자석의 같은 극처럼 서로 밀어냅니다. 별마다 크기가 다르므로 충돌 거리도 다릅니다. 본문이 길거나 태그가 많으면 더 큰 별이 되고, 고정 메모는 더 큰 황금 항성이 됩니다. 최대 180번 자리를 조정한 뒤 일반 물리 이동은 멈춥니다.
...
**5단계 — 우주 도화지에 그리기.** [orbitCanvas.ts](src/lib/orbitCanvas.ts)의 `drawOrbitCanvas`가 위치를 받아 Canvas라는 픽셀 전용 고속 도화지에 성운·별·연결선을 그립니다. 선은 스카이블루에서 황금색으로 이어지는 그라데이션과 은은한 빛을 사용합니다. 공통 태그가 많으면 선이 굵어지지만, 너무 복잡한 거미줄이 되지 않도록 별 하나당 강한 선 네 개까지만 보여 줍니다.

고정 메모는 황금빛이 커졌다 작아지는 Pulse 항성입니다. 사용자가 별에 마우스를 올리거나 터치해 선택하면 그 별과 직접 이어진 이웃만 밝아지고 나머지는 어두워집니다. 어두운 극장에서 무대 조명이 주인공과 바로 옆 배우만 비추는 것과 같습니다. 움직임 감소 설정을 사용하는 기기에서는 불필요한 애니메이션을 줄입니다.

`transformRef`는 도화지를 어느 방향으로 얼마나 옮기고 확대해서 볼지 기억합니다. `ResizeObserver`는 PC 창 크기나 모바일 화면 크기가 변하면 새 도화지 크기를 재고 다시 중앙에 맞춥니다. 확대·이동·화면 크기 조정은 메모 본문이나 DB를 수정하지 않습니다.

**6단계 — 멀리서는 성운 이름, 가까이서는 메모 제목 보기.** 화면을 멀리 보면 각 무리의 중심에 `#개발 (15)`처럼 대표 태그와 메모 수가 보입니다. 이것이 LOD입니다. 지도를 멀리 볼 때 도시 이름만 보이고 가까이 확대하면 도로 이름까지 보이는 방식과 같습니다. 배율이 1.05를 넘으면 개별 메모 제목이 서서히 나타나고, 1.7 근처에서 또렷해집니다.

**7단계 — 검색과 태그 칩으로 카메라 이동하기.** 위쪽 Glass 검색바에서 제목·본문·태그를 찾거나 `#개발`, `#독서` 같은 주요 태그 칩을 누릅니다. `focusTag`와 `focusMemoGroup`이 해당 별들의 평균 위치를 구하고 `animateCamera`가 약 360ms 동안 그곳으로 미끄러집니다. 이 과정은 메모를 검색해 보여 줄 뿐 DB 내용을 바꾸지 않습니다.

**8단계 — 성운의 메모 묶음 열기.** 별이나 `#개발 (15)` 레이블을 누르면 `openCluster`가 같은 성운의 메모 번호를 모읍니다. 오른쪽에서 Discovery 패널이 미끄러져 들어오고 제목·날짜·태그·대표 사진이 있는 카드가 세로로 나타납니다. 카드를 누르면 부모의 `onOpenMemo`가 기존 작성·수정 모달을 엽니다. 새 저장소나 별도 Zustand Store를 만들지 않고 이미 `Home`이 가진 메모를 그대로 사용합니다.

AI 연결이 필요한 경우에는 `onLinksAnalyzed`를 통해 `Home`의 `applyAnalyzedLinks`로 결과를 전달합니다. `memos`에 연결 정보를 반영하면 저장 이펙트가 이를 DB에도 기록합니다. AI 분석에 실패하면 기존 메모와 저장된 연결을 유지하고, 화면 전용 로컬 별자리와 짧은 실패 안내·재시도 버튼을 사용합니다. 도화지가 직접 서버 분석이나 메모 저장까지 하는 것은 아닙니다.
