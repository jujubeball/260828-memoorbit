  ...
  return (
    <section className="mb-8" aria-labelledby="memory-orbit-title">
      <div className="flex h-11 items-center justify-between px-1">
        ...
        {isOpen && <span className="text-xs text-[#8e8e93]">기억이 다시 도착한 날</span>}
      </div>
      {isOpen && (
        <div id="memory-orbit-content" className="flex snap-x gap-4 overflow-x-auto px-1 pb-4 pt-2">
          {candidates.map(({ memo, intervalLabel }) => (
            <button
              ...
              className="ios-tap w-[min(82vw,340px)] shrink-0 snap-center rotate-[-1deg] bg-white p-3 pb-6 text-left shadow-lg"
            >
              ...
              <div className="px-1 pt-4">
                ...
                <p className="mt-3 line-clamp-2 text-sm leading-6 text-[#636366]">{memo.content}</p>
              </div>
            </button>
          ))}
        </div>
