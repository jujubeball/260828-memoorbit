...
export function ResponsiveDatePicker({ id, label, value, onChange }: ResponsiveDatePickerProps): React.JSX.Element {
  ...
  return (
    <div className="relative grid gap-2">
      ...
      {isOpen && (
        <>
          <div className="absolute left-0 top-full z-50 mt-2 hidden w-80 rounded-2xl border border-[#2a2e3d] bg-[#1a1d26]/95 p-4 text-[#f3f4f6] shadow-2xl backdrop-blur-md xl:block" role="dialog" aria-label={`${label} 달력`}>{calendar}</div>
          <div className="fixed inset-0 z-[150] flex items-end bg-black/60 p-4 xl:hidden" role="dialog" aria-modal="true" aria-label={`${label} 달력`}><div className="w-full rounded-3xl border border-[#2a2e3d] bg-[#1a1d26]/95 p-5 text-[#f3f4f6] shadow-2xl backdrop-blur-md"><div className="mb-4 flex justify-between"><strong>{label}</strong><button type="button" onClick={closePicker} className="glass-icon-button" aria-label="달력 닫기">×</button></div>{calendar}</div></div>
        </>
      )}
    </div>
  );
