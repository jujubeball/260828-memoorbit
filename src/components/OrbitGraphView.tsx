  ...
  return (
    <section className="text-[#f3f4f6]" aria-labelledby="orbit-graph-title">
      ...
      <div className="glass-panel relative mt-5 hidden h-[650px] overflow-hidden xl:block">
        ...
        {selectedTag && <div className="absolute inset-x-4 bottom-4 flex gap-2 overflow-x-auto rounded-2xl border border-[#2a2e3d] bg-[#0f1117]/80 p-3 backdrop-blur-md">{relatedMemos.map((memo) => <button key={memo.id} type="button" onClick={() => onOpenMemo(memo)} className="shrink-0 rounded-xl bg-white/5 px-4 py-3 text-left hover:bg-white/10"><strong className="block max-w-52 truncate text-sm">{memo.title}</strong><span className="text-xs text-[#9ca3af]">{memo.tags.map((tag) => `#${tag}`).join(" ")}</span></button>)}</div>}
      </div>
      ...
      {isSheetOpen && selectedTag && <div className="fixed inset-0 z-[140] flex items-end bg-black/60 p-3 xl:hidden" role="dialog" aria-modal="true" aria-labelledby="orbit-sheet-title"><div className="max-h-[70dvh] w-full overflow-y-auto rounded-3xl border border-[#2a2e3d] bg-[#1a1d26]/95 p-5 backdrop-blur-md"><div className="flex items-center justify-between"><h3 id="orbit-sheet-title" className="text-xl font-bold">#{selectedTag} 궤도의 메모</h3><button type="button" onClick={() => setIsSheetOpen(false)} className="glass-icon-button" aria-label="연관 메모 닫기">×</button></div><div className="mt-4 grid gap-2">{relatedMemos.map((memo) => <button key={memo.id} type="button" onClick={() => onOpenMemo(memo)} className="rounded-xl border border-[#2a2e3d] bg-white/5 p-4 text-left"><strong>{memo.title}</strong><p className="mt-1 line-clamp-2 text-sm text-[#9ca3af]">{memo.content}</p></button>)}</div></div></div>}
    </section>
  );
}
